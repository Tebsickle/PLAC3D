...
const rebuildChunk = (key: string) => {
  ...
  for (const [color, items] of byColor) { const visibleItems = items.filter((voxel) => !erasedPositions.has(`${voxel.x},${voxel.y},${voxel.z}`)); const fadedItems = items.filter((voxel) => erasedPositions.has(`${voxel.x},${voxel.y},${voxel.z}`)); for (const [subset, opacity] of [[visibleItems, 1], [fadedItems, 0.08]] as const) { if (!subset.length) continue; const mesh = new THREE.InstancedMesh(cube, new THREE.MeshStandardMaterial({ color: paletteColor(color), roughness: 0.82, transparent: opacity < 1, opacity }), subset.length); const matrix = new THREE.Matrix4(); subset.forEach((voxel, index) => { matrix.setPosition(voxel.x - WORLD_SIZE / 2 + 0.5, voxel.y + 0.5, voxel.z - WORLD_SIZE / 2 + 0.5); mesh.setMatrixAt(index, matrix) }); mesh.castShadow = true; mesh.receiveShadow = true; group.add(mesh) } }
